...
// Lazy-loaded pose module ΓÇö MediaPipe only when Assessment entered per spec
...
export default function Assessment() {
  ...
  // Lazy-load MediaPipe pose only when Assessment entered ΓÇö per spec performance requirement
  useEffect(() => {
    let cancelled = false
    const load = async () => {
      ...
      try {
        // Dynamic import ΓÇö heavy @mediapipe/tasks-vision only loaded here
  ...
  // Pose inference loop ΓÇö only when camera running
  useEffect(() => {
  ...
  const getCameraErrorMessage = () => {
    switch (cameraState) {
      ...
      case 'security-error':
        return {
          title: t('screening.assessment.camera.securityError') || 'Security error ΓÇö HTTPS required',
      ...
      <main className="flex-1 flex flex-col px-4 pt-3 pb-6 page-enter">
        ...
        {phase === 'idle' && true && (
          <div className="mt-3 card p-4 bg-info-tint/30">
            ...
            {!poseLoaded && <p className="mt-2 text-[12px] text-warning-text">Loading pose modelΓÇª Please wait. (Lazy-loaded on entry)</p>}
            {poseLoaded && !personDetected && <p className="mt-2 text-[12px] text-warning-text">No person detected ΓÇö adjust position to show full body.</p>}
            {poseLoaded && personDetected && !stableDetection && <p className="mt-2 text-[12px] text-info">Person detected ΓÇö hold steady for ready state.</p>}
            {stableDetection && <p className="mt-2 text-[12px] text-primary font-semibold">Γ£ô Stable detection ΓÇö ready to start assessment.</p>}
          </div>
        )}
          ...
          <div className="card mt-3 overflow-hidden">
            <div className="relative bg-black aspect-[4/3] w-full overflow-hidden">
              ...
              {cameraState === 'requesting' && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white">
                  ...
                  <p className="mt-2 text-sm break-words">Requesting camera accessΓÇª</p>
                </div>
              )}
              ...
              {cameraState === 'starting' && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white">
                  <Camera size={32} className="animate-pulse" />
                  <p className="mt-2 text-sm break-words">Starting cameraΓÇª</p>
                </div>
              )}
              {cameraState === 'denied' && (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-white p-4 text-center">
                  ...
                  <p className="text-[11px] mt-2 opacity-60 break-words">To allow: Browser settings ΓåÆ Privacy ΓåÆ Camera ΓåÆ Allow this site</p>
                </div>
              )}
              ...
              {!poseLoaded && !poseError && (
                <div className="absolute bottom-2 left-2 bg-black/60 text-white text-[11px] px-2 py-1 rounded">Loading pose modelΓÇª</div>
              )}
              {poseError && (
              ...
              {poseLoaded && !personDetected && phase === 'recording' && (
                <div className="absolute bottom-2 left-2 bg-warning-tint text-warning-text text-[11px] px-2 py-1 rounded flex items-center gap-1">
                  ...
                  {t('screening.assessment.camera.noPerson')} ΓÇö Adjust position
                </div>
              )}
              ...
              <div className="absolute top-2 left-2 bg-black/60 text-white text-[10px] px-2 py-1 rounded">
                {facingMode === 'environment' ? 'Rear' : 'Front'} ┬╖ {videoRef.current?.videoWidth || 0}├ù{videoRef.current?.videoHeight || 0}
              </div>
            </div>
            ...
            {showDiagnostics && (
              <div className="p-3 bg-black text-green-400 text-[10px] font-mono leading-tight break-words">
                ...
                <div>Video: {videoRef.current?.videoWidth}├ù{videoRef.current?.videoHeight} readyState: {videoRef.current?.readyState} paused: {videoRef.current?.paused ? 'yes' : 'no'}</div>
          ...
          <div className="mt-3">
            <Callout tone="error" title={cameraErrorInfo.title}>
              ...
              {cameraErrorInfo.showPermissionGuidance && (
                <div className="mt-2 text-[12px]">Please allow camera permission in browser settings and retry. On mobile: Settings ΓåÆ Privacy ΓåÆ Camera ΓåÆ Allow.</div>
              )}
            </Callout>
          </div>
        )}
            ...
            <>
              ...
              {phase === 'cameraError' && (
                <Button full variant="ghost" onClick={() => { stopCamera(); setMode('simulated'); restart() }} className="min-h-[44px]">
                  {t('screening.assessment.camera.continueDemo')} ΓÇö Demo / Simulated Assessment
                </Button>
              )}
