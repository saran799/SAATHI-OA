...
export default function Records() {
  ...
  const getSyncDisplay = (sync: SyncState) => {
    switch (sync) {
      case 'local':
        return { icon: HardDrive, label: t('records.local'), cls: 'bg-tint text-secondary', sub: t('sync.localSub', { count: 1 } as any) || 'Saved locally' }
      case 'unsynced':
        return { icon: CloudOff, label: t('records.pending'), cls: 'bg-warning-tint text-warning-text', sub: 'Waiting for connection' }
      case 'syncing':
        return { icon: RefreshCw, label: t('sync.syncing'), cls: 'bg-info-tint text-info', sub: 'Syncing…' }
      case 'synced':
        return { icon: Cloud, label: t('records.synced'), cls: 'bg-tint text-secondary', sub: 'Synced' }
      case 'error':
      case 'failed':
        return { icon: AlertTriangle, label: t('sync.failed'), cls: 'bg-error-tint text-error-text', sub: 'Sync failed — Retry' }
      default:
        return { icon: CloudOff, label: t('records.pending'), cls: 'bg-warning-tint text-warning-text', sub: '' }
    }
  }
  ...
  return (
    <AppShell title={t('common.appName')} subtitle={t('records.title')}>
      <div className="pt-4 flex items-center gap-3">
        <h1 className="text-[22px] font-bold tracking-tight whitespace-nowrap break-words">{t('records.title')}</h1>
      ...
      <div className="mt-2 flex items-center justify-between text-[12px] font-medium"><span className="text-secondary inline-flex items-center gap-1.5 break-words"><span className="h-1.5 w-1.5 rounded-full bg-primary" aria-hidden />{t('records.outreach')}</span><span className="text-primary break-words">{t('records.stored')}</span></div>

      <div className="relative mt-3">
        <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-secondary" aria-hidden />
        <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('records.placeholder')} aria-label="Search records" className="w-full h-[52px] pl-11 pr-4 rounded-[14px] bg-surface shadow-[var(--shadow-card)] text-[15px] focus:outline-none focus:ring-2 focus:ring-primary" />
      </div>
      <div className="mt-3 flex gap-2 overflow-x-auto -mx-4 px-4 pb-1 [scrollbar-width:none]" role="tablist" aria-label="Filter">
        {filters.map(k => (
          <button key={k} role="tab" aria-selected={filter === k} onClick={() => setFilter(k)} className={cx('h-10 px-3.5 rounded-full text-[13px] font-semibold whitespace-nowrap inline-flex items-center gap-2 shrink-0 break-words min-h-[40px]', filter === k ? 'bg-primary text-white' : 'bg-surface text-ink shadow-[var(--shadow-card)]')}>
            ...
            <button key={r.id} type="button" onClick={() => nav(`/records/${r.id}`)} className={cx('w-full card p-3.5 text-left hover:bg-tint/40 transition-colors', r.result.band === 'higher' && 'border-l-4 border-error')}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[17px] font-bold leading-tight truncate break-words">{p.name} <span className="text-[13px] font-medium text-secondary">({p.age}{p.sex[0]})</span></p>
                  <p className="text-[13px] text-secondary mt-0.5 truncate break-words">{jointLabel}</p>
                </div>
                <div className="text-right shrink-0">
                  <span className={cx('h-6 px-2 rounded-full text-[11px] font-semibold inline-flex items-center gap-1 break-words', syncInfo.cls)}>
              ...
              {(r.sync === 'local' || r.sync === 'error' || r.sync === 'failed') && (
                <p className="mt-2 text-[11px] text-secondary break-words">{r.sync === 'local' ? t('sync.savedLocalLong') || 'Saved locally and waiting for connection' : t('sync.failedSub') || 'Sync failed — tap to retry'}</p>
              )}
            </button>
          )
        })}
